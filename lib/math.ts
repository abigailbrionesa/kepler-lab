...
//radians
export function kepler_solve(
  eccentricity: number,
  mean_anomaly_at_selected_date: number,
  decimals: number,
): number {
  const maxIter = 30;
  let i = 0;
  const delta = Math.pow(10, -decimals);
  const normalized_mean_anomaly = normalize_radians(
    mean_anomaly_at_selected_date,
  );
  ...
  F =
    eccentric_anomaly -
    eccentricity * Math.sin(normalized_mean_anomaly) -
...
export const get_true_anom = (
  eccentricity: number,
  eccentric_anomaly: number,
  decimals: number,
): number => {
...
export function get_position_at_selected_date(
  ...
  const mean_anomaly_at_selected_date =
    mean_anomaly + mean_motion * time_since_epoch;
  const eccentric_anomaly_at_selected_date = -kepler_solve(
    eccentricity,
    mean_anomaly_at_selected_date,
    4,
  );
  const true_anomaly_at_selected_date = get_true_anom(
    eccentricity,
    eccentric_anomaly_at_selected_date,
    5,
  );
  const coordinates = keplerian_to_cartesian_coords(
    distance_from_sun,
    eccentricity,
    inclination,
    argument_of_periapsis,
    longitude_of_ascending_node,
    true_anomaly_at_selected_date,
  );
  return coordinates;
}
...
export function get_orbit_points(
  ...
  for (let true_anomaly = 0; true_anomaly < full_circle; true_anomaly += step) {
    const position = keplerian_to_cartesian_coords(
      distance_from_sun,
      eccentricity,
      inclination,
      argument_of_periapsis,
  ...
  const final_point = keplerian_to_cartesian_coords(
    distance_from_sun,
    eccentricity,
    inclination,
    argument_of_periapsis,
...
export function keplerian_to_cartesian_coords(
  ...
  const orbital_radius =
    (distance_from_sun * (1 - eccentricity * eccentricity)) /
    (1 - eccentricity * Math.cos(true_anomaly + pi));
  // polar coordinates
  const x = orbital_radius * Math.cos(true_anomaly);
  const y = orbital_radius * Math.sin(true_anomaly);
  // periapsis (ω)
  const x1 =
    x * Math.cos(argument_of_periapsis) + y * Math.sin(argument_of_periapsis);
  const y1 =
    -x * Math.sin(argument_of_periapsis) + y * Math.cos(argument_of_periapsis);
  // rotation (i)
  const z1 = y1 * Math.sin(inclination);
  const y2 = y1 * Math.cos(inclination);
  // ascending node (Ω)
  const x3 =
    x1 * Math.cos(longitude_of_ascending_node) +
    y2 * Math.sin(longitude_of_ascending_node);
  const y3 =
    -x1 * Math.sin(longitude_of_ascending_node) +
    y2 * Math.cos(longitude_of_ascending_node);
  return new Vector3(
